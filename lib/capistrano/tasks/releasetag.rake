...
namespace :load do
  task :defaults do
    ...
    set :release_repo_dir, -> { "release_repo" }
    set :release_repo_root_path, -> { "#{fetch(:deploy_to)}" }
    set :release_repo_path, -> { "#{fetch(:release_repo_root_path)}/#{fetch(:release_repo_dir)}" }
    set :release_tag_version, -> { "0.0.1" }
    set :release_tag_date, -> { true }
    set :release_tag_prefix, -> { "#{fetch(:stage)}-" }
    set :release_tag_suffix, -> { "" }
  end
end
